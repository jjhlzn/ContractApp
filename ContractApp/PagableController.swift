...
class PagableController<T> : NSObject {
    ...
    func checkIsNeedRefresh() -> Bool {
        if lastUpdateTime == nil {
            return false
        }
        let currentDateTime = NSDate()
        let delta = currentDateTime.timeIntervalSince1970 - lastUpdateTime!.timeIntervalSince1970
        print("delta = \(delta)")
    ...
    func afterHandleRefreshRespones(resp: PageServerResponse<T>) {
        print("afterHandleRefreshRespones")
        self.quering = false
        refreshControl.endRefreshing()
